...
function setDOM(playerDOM, player) {
  ...
  playerDOM.getElementsByClassName("number")[0].innerHTML = player.back_number;
...
function update() {
  ...
  if (controlPlayer.isMoving) {
    console.log("hello");
    walk_paths.map(function (item, idx) {
      if (idx == 0) {
        // TODO
        shapeIndex = 10;
      } else if (idx == 4) {
      ...
      gsap.to(item, {
        ...
        repeat: -1,
        yoyo: true,
        morphSVG: { shape: walk2_paths[idx], shapeIndex: shapeIndex },
      });
    });
  }
  ...
  console.log(controlPlayer.isMoving);
}
